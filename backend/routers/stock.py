...
@router.post('/live/soft-delete-row', dependencies=[Depends(require_admin)])
def soft_delete_live_stock_row(payload: StockLiveReturnRequest, runtime=Depends(get_runtime), current_user=Depends(require_admin)):
    try:
        result = runtime.soft_delete_stock_row(payload.row_num, force_refresh=payload.force_refresh)
    except RuntimeError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
